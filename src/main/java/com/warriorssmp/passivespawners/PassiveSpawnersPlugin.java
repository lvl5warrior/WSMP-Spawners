...
public class PassiveSpawnersPlugin extends JavaPlugin {
    ...
    // Caps each of the three failure warnings below to firing ONCE per
    // distinct spawner location, ever (per server run) - without this, a
    // single persistently-failing spawner among many, on a server full of
    // players, would print a fresh warning every single generation cycle
    // forever, which is exactly the kind of console flooding this is meant
    // to prevent.
    private final java.util.Set<String> warnedFailureLocations = new java.util.HashSet<>();

    private boolean shouldWarnOnce(Location location, String category) {
        String key = category + ":" + location.getWorld().getName() + "," + location.getBlockX() + ","
                + location.getBlockY() + "," + location.getBlockZ();
        return warnedFailureLocations.add(key);
    }
    ...
    private void onGenerate(Location location, SpawnerData data) {
        ...
        try {
            ...
            if (shouldWarnOnce(location, "loot")) {
                getLogger().warning("[WSMP-PassiveSpawners] Loot generation/routing failed for a " + data.mobType
                        ...
                        + " (further failures at this exact spawner will be suppressed)");
            }
        }
        ...
        try {
            ...
            if (shouldWarnOnce(location, "persist")) {
