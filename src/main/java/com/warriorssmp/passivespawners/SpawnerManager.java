...
public class SpawnerManager {
    ...
    public SpawnerData readFromBlock(Block block) {
        ...
        java.util.List<org.bukkit.inventory.ItemStack> storedItems;
        String serializedItems = pdc.get(keys.blockStoredItems, PersistentDataType.STRING);
        try {
            storedItems = ItemSerialization.deserialize(serializedItems);
        } catch (Exception e) {
            plugin.getLogger().warning("Spawner at " + describeLocation(block.getLocation())
                    + " had corrupted stored-item data - starting with empty storage instead of losing "
                    + "the rest of the spawner's data. Error: " + e);
            storedItems = new java.util.ArrayList<>();
        }
        ...
        return new SpawnerData(mobType, level, storedXp, currentTick, storedItems);
    }
    ...
    public void writeToBlock(Block block, SpawnerData data) {
        ...
        pdc.set(keys.blockStoredItems, PersistentDataType.STRING, ItemSerialization.serialize(data.storedItems));
        spawner.update();
    }
